...
(function (window, mars2d) {
  ...
  class MyWidget extends mars2d.widget.BaseWidget {
    ...
    activate() {
      //监听事件，联动勾选状态
      // this.map.on(mars2d.EventType.layeradd, this._onAddLayerHandler, this);
      // this.map.on(mars2d.EventType.layerremove, this._onRemoveLayerHandler, this);
    }
    //关闭释放
    disable() {
      this.viewWindow = null;

      // this.map.off(mars2d.EventType.layeradd, this._onAddLayerHandler, this);
      // this.map.off(mars2d.EventType.layerremove, this._onRemoveLayerHandler, this);
    }
    ...
    updateLayerShow(layer, show) {
      ...
      if (show) {
        // if (!layer.isAdded) {
        this.map.off(mars2d.EventType.addLayer, this._onAddLayerHandler, this);
        this.map.addLayer(layer);
        this.map.on(mars2d.EventType.addLayer, this._onAddLayerHandler, this);
        // }
      } else {
        // if (layer.isAdded) {
