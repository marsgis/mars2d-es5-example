...
class PoiQueryButton extends mars2d.control.ToolButton {
  ...
  _initQueryUI() {
    // 高度值获取
    this.parentContainer = this._map._controlCorners.bottomright
    ...
    this._container.firstChild.className = "poi-search-image"
    ...
    this._queryResultContainer.style.height = this.parentContainer.offsetHeight
    this._container.appendChild(this._queryResultContainer)
    console.log(this._container, this.parentContainer.offsetHeight)
    this.toolSearchNoShow("none")
  ...
  flyTo(item) {
    ...
    if (graphic === null) {
      haoutil.Msg(item.name + " 无经纬度坐标信息！")
      return
    }
    // 预留功能，后续支持高亮操作
